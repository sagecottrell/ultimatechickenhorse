...
## the last time the player was on the floor
var _last_time_on_floor_tickms : int
...
func _coyote_time():
	return Time.get_ticks_msec() - coyote_time_ms < _last_time_on_floor_tickms
...
func move(_delta: float, input_axis := Vector2.ZERO, input_jump := false, input_crouch := false, input_sprint := false, input_swim_down := false, input_swim_up := false) -> void:
	...
	var coyote = _coyote_time()
	
	swim_ability.set_active(!fly_ability.is_actived())
	jump_ability.set_active(input_jump and coyote and not head_check.is_colliding())
	walk_ability.set_active(not is_fly_mode() and not swim_ability.is_floating())
	crouch_ability.set_active(input_crouch and coyote and not is_floating() and not is_submerged() and not is_fly_mode())
	sprint_ability.set_active(input_sprint and coyote and  input_axis.y >= 0.5 and !is_crouching() and not is_fly_mode() and not swim_ability.is_floating() and not swim_ability.is_submerged())
	...
	for ability in _abilities:
		velocity = ability.apply(velocity, speed, coyote, direction, _delta)
